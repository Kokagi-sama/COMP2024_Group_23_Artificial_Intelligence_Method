...
public class SimulatedAnnealing {
    ...
    private final Map<String, Solution> initialSolution;

    public SimulatedAnnealing(Heuristics heuristic, Map<String, Solution> initialSolution) {
        this.heuristic = heuristic;
        this.initialSolution = initialSolution;
    }

    public Map<String, Solution> applySimulatedAnnealing() {
        double temperature = START_TEMPERATURE;
        Map<String, Solution> currentSolution = heuristic.copySolution(initialSolution);
        Map<String, Solution> bestSolution = heuristic.copySolution(initialSolution);

        while (temperature > 1) {
            for (int i = 0; i < ITERATIONS_PER_TEMPERATURE; i++) {
                Map<String, Solution> newSolution = heuristic.generateNeighbor(currentSolution);
